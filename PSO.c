...
int main(){
    ...
    int n_part = 50;        /* Número de partículas */
    ...
    for(i = 0; i < n_part; i++){
        for(j = 0; j < n_var; j++){
            position[i][j] = ((double) rand()/(double) RAND_MAX); /* Atribui valores randomicos entre 0 e 1 */
        }
    }
    ...
    while ((ite < iteracoes) && target_error>10^-2){
        
        /* Atualização da matriz velocidade */
        for(i = 0; i < n_part; i++){
            for(j = 0; j < n_var; j++){
                inercial = w*veloc[i][j];
                social = c1* ((double) rand()/(double) RAND_MAX)*(pbest[i][j]-position[i][j]);
                cognitivo = c2* ((double) rand()/(double) RAND_MAX)*(gbest[j]-position[i][j]);
